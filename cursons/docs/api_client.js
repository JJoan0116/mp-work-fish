 ...
 * // 添加摸鱼记录
 * try {
 *   const newRecord = await api.addWorkFish({
 *     userId: 'user123',
 *     time: '14:30',
 *     duration: 30,
 *     content: '看视频摸鱼'
 ...
 * // 查询当天摸鱼记录
 * try {
 *   const todayRecords = await api.getTodayWorkFish('user123');
 *   console.log('今日摸鱼记录:', todayRecords.data);
 *   console.log('记录数量:', todayRecords.count);
 * } catch (error) {
 ...
 *     workdays: ['1', '2', '3', '4', '5']
 *   });
 *   console.log('设置更新成功:', settings.data);
 * } catch (error) {
 ...
 * // 获取用户设置
 * try {
 *   const settings = await api.getSettings('user123');
 *   console.log('用户设置:', settings.data);
 * } catch (error) {
